...
export class Service implements Resource {
  ...
  constructor(cluster: Cluster, opts: any) {
    ...
    this.healthcheckPath =
      opts.healthcheckPath || this.DEFAULT_HEALTHCHECK_PATH;
  ...
  requirePort() {
    throw new TypeError(
      "Service definition requires a Port when mapping a URL"
    );
  }
  ...
  requireURL() {
    throw new TypeError(
      "Service definition requires a URL when mapping a Port"
    );
  }
  ...
  get name() {
    const serviceStage = this._stage
      ? `${this._service}-${this._stage}`
      : this._service;

    return _.chain(`${serviceStage}-${this._name}`)
      .camelCase()
      .upperFirst()
      .value();
  }
