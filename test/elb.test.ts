...
describe("with an existing cluster", () => {
  ...
  class ELBTest {
    opts = {
      id:
        "arn:aws:ecs:ap-southeast-2:000000000001:cluster/ECSCluster-XXXXXXXXXXXXX",
    ...
    before() {
      let cluster = new Cluster(this.opts, "TestName");
...
describe("creating a new cluster with HTTP", () => {
  ...
  class ELBTest {
    ...
    elb_listener_certificate() {
      let result = _.get(
        this.resources,
        "ClsHTTPListener.Properties.Certificates[0].CertificateArn"
      );
